...
@section('content')
    <div class="mb-4">
        <h1 class="xai-display">Payouts</h1>
        <p class="xai-subheading">Review withdrawal requests and mark payments as complete.</p>
    </div>
                ...
                <tbody>
                    @forelse($payouts as $payout)
                        <tr class="align-middle">
                            <td class="ps-4">
                                <div style="font-weight: 400; color: var(--xai-text-primary); font-size: 14px;">{{ $payout->created_at->format('M d, Y') }}</div>
                                <div style="font-size: 11px; color: var(--xai-text-muted); font-family: var(--font-display);">{{ $payout->created_at->format('H:i') }} UTC</div>
                            </td>
                            <td>
                                <div class="d-flex align-items-center gap-3">
                                    ...
                                    <div>
                                        <div style="font-weight: 400; color: var(--xai-text-primary); font-size: 13px;">{{ $payout->affiliate->user->name ?? 'Unknown' }}</div>
                                        <div style="font-size: 10px; color: var(--xai-text-muted);">{{ $payout->affiliate->user->email ?? 'No email' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <div style="font-family: var(--font-display); font-weight: 400; color: var(--xai-text-primary); font-size: 15px;">${{ number_format($payout->amount, 2) }}</div>
                            </td>
                            <td>
                                <div class="d-flex flex-column">
                                    <span style="color: var(--xai-text-primary); font-family: var(--font-display); font-size: 12px;">
                                    ...
                                    <div style="font-size: 10px; color: var(--xai-text-muted);">
                                        ...
                                        @elseif($payout->payment_method === 'crypto')
                                            <span style="font-family: var(--font-display);">{{ Str::limit($details['address'] ?? 'N/A', 12) }}</span>
                                        @elseif($payout->payment_method === 'bank_transfer')
                                            {{ $details['bank_name'] ?? 'Bank' }} / {{ $details['account_number'] ?? 'N/A' }}
                                        @endif
                                    </div>
                                </div>
                            </td>
                            <td>
                                @if($payout->status === 'completed')
                                    <span style="font-family: var(--font-display); font-size: 10px; color: var(--atlas-success); letter-spacing: 0.5px; border: 1px solid var(--atlas-success); padding: 2px 8px;">Completed</span>
                                @elseif($payout->status === 'processing')
                                    <span style="font-family: var(--font-display); font-size: 10px; color: var(--xai-text-primary); letter-spacing: 0.5px; border: 1px solid var(--xai-border-strong); padding: 2px 8px;">Processing</span>
                        ...
                        <tr>
                            <td colspan="6" class="text-center py-5">
                                <div style="color: var(--xai-text-muted);">
                                    <p style="font-family: var(--font-display); font-size: 12px;">No payout requests found</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
